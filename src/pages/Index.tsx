    ...
    <div className="min-h-screen flex flex-col">
      <Helmet>
        <title>
          ...
          Daily Overwatch Hero Guessing Game
        </title>
        <meta
          name="description"
          content={`Play Overwadle in ${currentMode} mode - Can you guess today's Overwatch hero? A daily hero guessing puzzle for Overwatch fans.`}
        />
        <meta
          name="keywords"
          content={`Overwadle, Overwatch game, ${currentMode} mode, hero guessing, daily puzzle`}
        />
        <link
          rel="canonical"
          href={`https://overwadle.online/#/${currentMode === 'classic' ? '' : currentMode}`}
        />
      </Helmet>
      <Header />
      <main className="flex-1 z-2 relative">{renderGameMode()}</main>
      <img
        src={overwatchBG}
      ...
      <footer className="py-2 md:py-4 text-center text-muted-foreground text-xs md:text-sm">
        ...
        <p className="text-[10px] md:text-xs">
          This game is fan-made and not affiliated with Blizzard Entertainment
        </p>
      </footer>
    </div>
  );
