...
import { getDailyQuoteByHeroId } from '../../data/quotes';
...
const QuotesMode = () => {
  ...
  useEffect(() => {
    if (targetHero) {
      setCurrentQuote(getDailyQuoteByHeroId(targetHero.id));
    }
  }, [targetHero]);
    ...
    <div className="container mx-auto px-2 md:px-4 py-4 md:py-8" ref={containerRef}>
      <div className="flex flex-col items-center space-y-4 md:space-y-6">
        <div className="bg-card border border-muted rounded-lg p-4 md:p-6 w-full max-w-2xl text-center">
          <h2 className="text-xl md:text-2xl font-bold mb-4">Who said this?</h2>
          <div className="relative">
            <span className="absolute top-0 left-0 text-4xl md:text-6xl opacity-50">"</span>
            <p className="text-lg md:text-xl px-8 py-4 italic font-medium">{currentQuote}</p>
            <span className="absolute bottom-0 right-0 text-4xl md:text-6xl opacity-50">"</span>
          </div>
        </div>
